...
export class HideHeaderDirective implements AfterViewInit {
  ...
  @HostListener('ionScroll', ['$event']) onContentScroll($event: any) {
    console.log('get the event inScroll', $event.detail.currentY);
    ...
    // If the direction changed, stroe the point of change for calculation
  ...
  ngAfterViewInit(): void {
    ...
    this._renderer.setStyle(this.content, 'position', `absolute`);
    this._renderer.setStyle(this.content, 'top', `${this.scrollDistance}px`)

    //Add the safe area top to complete fade out the header
